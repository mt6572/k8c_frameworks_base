...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private final Runnable mPowerLongPress = new Runnable() {
		...
		Intent intent = new Intent();  
				//SystemProperties.set("persist.service.hallcamera", "0");
	       intent.setAction( "android.intent.action.POWEROFF_DIALOG");
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
       // final
	 boolean keyguardOn = false;// keyguardOn();
	      ...
	      if(down)
	    {
		  ...
		  int kk = 0;
	       ITelephony telephonyService = getTelephonyService();
			...
			//Log.d(TAG,"22222 2222 Start"+kk);
                if (kk == 0) {
                   Intent intent = new Intent();  
		      intent.setAction( "android.intent.action.KEY_SOS");
		      intent.setFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
		      mContext.sendBroadcast(intent);	
                 }
	             /*Intent intent = new Intent();  
		...
		{
		  // if((repeatCount == 50)&&(down))
                //  toggleRecentApps();
		 //  else
               // cancelPreloadRecentApps();


               if((repeatCount == 35)&&(down))
		   {
	            // mHomeConsumed = true;
		  int kk = 0;
	       ITelephony telephonyService = getTelephonyService();
	...
	else if (keyCode == KeyEvent.KEYCODE_SOS) {
            if (down) {
				int kk = 0;
	       ITelephony telephonyService = getTelephonyService();
		     if (telephonyService != null) {
			...
			Log.d(TAG,"22222 2222 Start"+kk);
                if ((repeatCount == 30)&&(kk == 0)) {
                    Log.d(TAG,"22222 InjectEvent Start");
                   Intent intent = new Intent();  
				//SystemProperties.set("persist.service.hallcamera", "0");
		      intent.setAction( "android.intent.action.KEY_SOS");
		      intent.setFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
    ...
    public void beginLayoutLw(boolean isDefaultDisplay, int displayWidth, int displayHeight,
		...
		{
		 Log.d(TAG, "zcj, mFocusedWindow" + mFocusedWindow + " type:" + mFocusedWindow.getAttrs().type+"title"+mFocusedWindow.getAttrs().getTitle().toString());
		
         if(mFocusedWindow.getAttrs().type !=FIRST_SYSTEM_WINDOW){
			mLayoutDisableStatusBar =(mFocusedWindow.getAttrs().type == TYPE_KEYGUARD 
		 				||(mFocusedWindow.getAttrs().packageName != null && 
						!(mFocusedWindow.getAttrs().getTitle().toString().equals("com.android.launcher/com.android.launcher2.Launcher"))));
			}
		}
		 Log.d(TAG, "zcj, mLayoutDisableStatusBar" + mLayoutDisableStatusBar );
