...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    private int mDownX;//zcj add for MotionEvent.ACTION_DOWN x
	...
	//zcj add start 20140625
	void sendEvent(int action,int mcode, long stime) {
	 Log.i(TAG, "phonewindow:zzzzInput event!"+",mcode = " +mcode);
		sendEvent(action, 0,mcode, stime);
	}
	
	void sendEvent(int action, int flags,int mcode, long start) {
		final int repeatCount = (flags & KeyEvent.FLAG_LONG_PRESS) != 0 ? 1 : 0;
		final KeyEvent ev = new KeyEvent(start,SystemClock.uptimeMillis(), action, mcode, repeatCount,
    ...
    private final class DecorView extends FrameLayout implements RootViewSurfaceTaker {
        ...
        public boolean dispatchTouchEvent(MotionEvent ev) {
            final Callback cb = getCallback();
			//zcj add start 20140625
			//filter the back and meun key
			
			int action = ev.getAction();
			int width= getWidth();
			if (action == MotionEvent.ACTION_DOWN){
                mDownX = (int)ev.getX();
                Log.i(TAG, "phonewindow:zzzzzzdown!"+",x = " + ev.getX());
            }
            else if (action == MotionEvent.ACTION_MOVE) {
				if(mDownX < 10 || mDownX > (width - 15)) return true;
            }
